...
namespace CboePitch {
    std::shared_ptr<Message> MessageFactory::parseMessage(const uint8_t *data, size_t size) {
        ...
        return it->second.parser(data, size, 0);
    }
    ...
    std::vector<std::shared_ptr<Message> > MessageFactory::parseMessages(const uint8_t *data, size_t length) {
        ...
        for (uint8_t i = 0; i < header.getCount() && offset < length; ++i) {
            ...
            std::cout << "messageLength: " << std::to_string(messageLength) << std::endl;
            std::cout << "messageLength: " << messageLength << std::endl;
            std::cout << "messageType: " << byteToHex(messageType) << std::endl;
            auto it = dispatchTable.find(messageType);
            if (it == dispatchTable.end()) {
            ...
            std::shared_ptr<Message> msg = dispatchInfo.parser(data, remainingLength, offset);
